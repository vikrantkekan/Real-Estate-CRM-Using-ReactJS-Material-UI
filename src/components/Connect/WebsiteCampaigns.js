...
function handlerowschange(e){
  ...
  function handle_add_modal(v){
    if(cookies.all.uroles[0].website_campaign.indexOf("add_webcampaign")!=-1){
    if(addmodal===0){
      setAddmodal(1)
    }else{
      setAddmodal(0)
...
useEffect(()=>{
  if(cookies.all.uroles[0].website_campaign){
fetch('http://localhost//demo/demo/php/website_campaigns.php',{
  method:'post',
...
<TableCell align="left" sx={{color:'#575757'}}>
<Chip label={item.created_date} color="success" variant="outlined"></Chip>
</TableCell>
...
<span style={{float:'left',margin:'25px',marginTop:'35px',fontSize:'18px',fontWeight:600}}>Website Campaigns</span>
...
<TableCell align="left" sx={{backgroundColor:'#f4f6f8', color:'#637381'}}>Website Link</TableCell>
<TableCell align="left" sx={{backgroundColor:'#f4f6f8', color:'#637381'}}>API Secret</TableCell>
<TableCell align="left" sx={{backgroundColor:'#f4f6f8', color:'#637381'}}>Created Date</TableCell>
