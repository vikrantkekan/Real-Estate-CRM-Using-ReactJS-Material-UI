...
function EditStatus(props){
    ...
    const [stage,setStage]=useState(props.data.stag)
    const [stages,setStages]=useState([]) 
    const [statusname,setStatusname]=useState(props.data.statu)
...
setStatus(e.target.value)
  ...
  function submit_form(e){
    ...
    let body_data={"status":status,"stage":stage,"statusname":statusname,"id":props.data.id}
  
  fetch('http://localhost//demo/demo/php/edit_status.php',{
    method:'post',
  ...
  .then((data)=>{
    ...
    if(data==200){
      setSnack(1)
      setSnack({"status":1,"severity":"success","msg":"Edited Successfuly !"})
    }else if(data==201){
...
<p style={{marginLeft:'20px',fontSize:'18px',color:'#193b68'}}>Edit Lead Status</p>
...
<Grid item xs={6}>
<FormControl sx={{p:2,width:'100%'}} >
<TextField label="Status Name" placeholder="Status Name" sx={{m:1}} defaultValue={props.data.statu} onChange={(e)=>setStatusname(e.target.value)}></TextField>
</FormControl>
</Grid>
...
<Grid item xs={12}>
<FormControl sx={{p:2}} >
<Button type="submit" variant="contained" sx={{m:1}} >Edit Status</Button>
</FormControl>
